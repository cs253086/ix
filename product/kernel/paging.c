...
debug_printf("page frame from 0x%x to 0x%x, ", proc_table[1].begin_phy_address, proc_table[1].end_phy_address);
	}
	else if (0 < component && component < USR)	//mm, fs, init
	{
		...
		for (;i < PAGE_SIZE / sizeof(uint32_t); i++)
        		page_table[i] = READ_WRITE | NOT_PRESENT;
debug_printf("page frame from 0x%x to 0x%x, ", proc_table[NUM_OF_TASKS + component].begin_phy_address, proc_table[NUM_OF_TASKS + component].end_phy_address);
	}
	else
	{
		i = 0;
		...
		for (;i < PAGE_SIZE / sizeof(uint32_t); i++)
        		page_table[i] = READ_WRITE | NOT_PRESENT;
debug_printf("page frame from 0x%x to 0x%x, ", proc_table[proc_table_idx].begin_phy_address, proc_table[proc_table_idx].end_phy_address);
		proc_table_idx++;
	}
