...
#include <libelf.h>
#include <gelf.h>
#include <stdint.h> #include "../h/common.h" 
#define KERNEL_COMMAND_IDX	2
#define MM_COMMAND_IDX	3
#define MBR_SIZE	512
#define OS_SIZE_SECTOR_OFFSET	444
...
void write_section(int fd, Elf *elf_handle, Elf_Scn *scn_cur, GElf_Shdr shdr_cur, char* section_name, int *section_size)
{
	...
	section_idx = elf_ndxscn(scn_cur);	// get section index
	gelf_getshdr(scn_cur, &shdr_cur);
	scn_next = elf_getscn(elf_handle, section_idx + 1);
	gelf_getshdr(scn_next, &shdr_next);	// get the next section header
...
void copy_component(char *file)
{
	...
	off_t offset;
    	...
    	if ((elf_handle = elf_begin(fd, ELF_C_READ, NULL)) == NULL)	// get elf_handle for the file
    	...
    	if (elf_getshdrstrndx(elf_handle, &shstrndx) != 0) // get section index of .shstrtab, which contains section names
        	err(EXIT_FAILURE , " elf_getshdrstrndx ()  failed : %s.", elf_errmsg ( -1));

	while ((scn_cur = elf_nextscn(elf_handle, scn_cur)) != NULL)	// get next section 
	{
		gelf_getshdr(scn_cur, &shdr_cur);	//get the section header
		cur_section_name = elf_strptr(elf_handle, shstrndx, shdr_cur.sh_name); 	// convert 'Elf32_Word sh_name' to 'char *'
		if (strcmp(cur_section_name, ".text") == 0)
		{
			// if it's not a kernel, need page alignment for the component before copying
			if (strcmp(file, "kernel/kernel") != 0)
			{
				// if it's mm, then page aligin to PROCESS_STARTING_VIR_ADDRESS
				if (strcmp(file, "mm/mm") == 0)
				{
					mm_gap_to_page = align_paging(PROCESS_STARTING_VIR_ADDRESS);
					total_gap_to_pages += mm_gap_to_page; 
				}
				// if it's others, then page alain to the next page
				else
		...
		else if (strcmp(cur_section_name, ".bss") == 0)
		{
			...
			if (strcmp(file, "mm/mm") == 0)
				server_stack_address[0] = (uint32_t) shdr_cur.sh_addr;	// address field of section (e.g readelf -S kernel/kernel)
...
// patch the total os size to bootblock
void patch_os_size_to_bootblock(uint16_t *os_size_sector)
{
	off_t os_size_offset = (off_t) OS_SIZE_SECTOR_OFFSET;	 
...
{
	int i;
	off_t kernel_data_offset = (off_t) (MBR_SIZE + sizes[KERNEL_COMMAND_IDX - 2]);	// 2 means build and bootloader
...
/*
 * patch text and data size of each program at the beginning address of mm .data section
 */
void patch_sizes_to_mm(uint32_t *sizes, int array_size)
{
	int i;
	off_t mm_data_offset = (off_t) (MBR_SIZE + sizes[KERNEL_COMMAND_IDX - 2] + sizes[KERNEL_COMMAND_IDX - 2 + 1] + mm_gap_to_page + sizes[(MM_COMMAND_IDX - 2) * 2]);	// 2 means build and bootloader
...
int main(int argc, char *argv[])
{
	...
	/* prepare patching useful info to os image */	
	...
	// page directory address must be page-aligned
	kernel_directory_address = (KERNEL_HEAD_ADDRESS + os_size_byte + PAGE_SIZE) & PAGE_ALIGN_FLAG;	// add extra PAGE_SIZE to make sure kernel_directory_address is > (KERNEL_HEAD_ADDRESS + os_size_byte)
