...
struct proc proc[NR_TASKS + NR_PROCS];	/* process table definition */
...
/* An interrupt has occurred.  Schedule the task that handles it. */
  ...
  /* Re-enable the 8259A interrupt controller. p.99 */
  ...
  /* See if any tasks that were previously busy are now listening for msgs that recorded in task_mess[] 
   * What if old_map == 0? then, message had to been sent successfully 
   */
  if (old_map != 0) {
        for (i = 2; i <= NR_TASKS; i++) {       /* It covers every task except HARDWARE */
                /* Check each task looking for one with a pending interrupt. */
                if ( (old_map >> i) & 1) {
                        /* Task 'i' has a pending interrupt. */
                        n = mini_send(HARDWARE, -i, task_mess[i]);
                        if (n == OK) busy_map &= ~(1 << i);     /* remove task 'i' from busy_map */
                }
        }
  }

  /* If a task has just been readied and a user is running, we need to pick a task over cur_proc Bug13 */
  if (rdy_head[TASK_Q] != NIL_PROC && (cur_proc >= 0 || cur_proc == IDLE))
        pick_proc();    /* call scheduler */
}

/*===========================================================================*
 *                              sys_call                                     * 
 *===========================================================================*/
sys_call(function, caller, src_dest, m_ptr)
 ...
 */
  ...
  /* Check to see if 'dest' is blocked waiting for this message. Refer to mini_rec() */
...
message *m_ptr;                 /* pointer to message buffer 
				 * For example, it's &mc at receive() in clock_task  
				 */
{
 ...
 */
  ...
  lock();                       /* disable interrupts. Why disabling here? 
				 * For example, clock_task receives a message and 
				 * the task is not put in ready queue due to another interrupt
				 * you may lose a clock tick 
				 */
  r = (rp - proc) - NR_TASKS;   /* task or proc number */
  q = (r < 0 ? TASK_Q : r < INIT_PROC_NR ? SERVER_Q : USER_Q);  /* Refer to important process numbers in h/const.h */
 ...
 */
  ...
  lock();                       /* disable interrupts. data-related work needs lock() (e.g. stack or linked list etc.) 
                                 * Because the data structure could be mssed. Think about when sched() happens consecutively
                                 * during 'rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];', for exmaple 
                                 */
  ...
  /* NOw, next process is the head in the queue */
