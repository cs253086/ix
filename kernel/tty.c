...
static struct tty_struct {
  ...
  char tty_makebreak;		/* 1 for terminals that interrupt twice/key. That is, pressed & released */
...
int scan_code;		/* scan code for '=' saved by bootstrap */
 ...
 *				tty_reply				     *
 *===========================================================================*/
static tty_reply(code, replyee, proc_nr, status, extra, other)
...
#define CONSOLE            0	/* line number for console. console means where the cursor is located and waiting for input */
...
/* A keyboard interrupt has occurred.  Process it. */
   ...
   * The shift-type keys, 29, 42, 54, 56, and 69 must be processed normally. Refer to p.178
   */
  ...
  if (k > 0) {
	...
	if (k != 29 && k != 42 && k != 54 && k != 56 && k != 69) {
		port_out(INT_CTL, ENABLE);	/* re-enable interrupts.
						 * the previous keyboard interrupt processing is done
						 * Refer to p.99
 						 */
	 ...
	 */
	raw_bit = tty_struct[0].tty_mode & RAW;
	stopc = tty_struct[0].tty_xoff;
	if (raw_bit == 0 && control && code == CTRL_S && stopc == XOFF_CHAR) {
		tty_struct[0].tty_inhibited = STOPPED;
