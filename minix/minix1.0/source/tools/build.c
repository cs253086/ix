 ...
 */
  ...
  count = (bytes <= room ? bytes : room); /* count = whatever it's not overflowed */
  count1 = count;
  p = &buf[buf_bytes]; 	/* destination: output buffer */
  q = buffer; 	/* src buffer */ 
  while (count--) *p++ = *q++;
  
  /* See if the buffer is full. */
  buf_bytes += count1; 	/* add num of bytes */  
  ...
  /* if count1 is bytes, not room. That is, if there si not any more data to copy, then return */
  if (count1 == bytes) return;
  /* if count1 == room which is bytes > room  */
  bytes -= count1;
  buf_bytes = bytes;
  p = buf;
  while (bytes--) *p++ = *q++;
 ...
 */
  ...
  fsck_org = PROG_ORG + cum_size;       /* where does fsck begins in abs memory */
  ip = 0;
  cs = fsck_org >> CLICK_SHIFT;	/* 20-bits memory to 16-bits memory to make it suitable for register */
  ...
  /* why SECTOR/2? it's because registers are 2bytes */
