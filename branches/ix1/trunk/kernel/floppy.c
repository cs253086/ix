...
#define MOTOR_OFF       3 * CLOCK_HZ	/* how long to wait before stopping motor */
...
#define NT                 4	/* number of diskette/drive combinations, which is density */
...
static int nr_sectors_track[NT]    = {9,    9,    9,    15};   /* sectors/track */
static int total_sectors[NT]     = {720,  720,  720,  2400}; /* sectors/diskette*/
static int steps_per_cyl[NT] = {1,    2,    2,    1};	  /* 2 = dbl step */
static int mtr_setup[NT]     = {CLOCK_HZ / 4, CLOCK_HZ / 4, 3 * CLOCK_HZ / 4, 3 * CLOCK_HZ / 4}; /* in ticks */
...
#ifdef DEBUG
        printf("floppy_task initialized...\n");
#endif
...
/* Carry out a read or write request from the disk. */
  ...
  long cur_sector;	/* current sector in total sectors */
  ...
  if (m_ptr->POSITION % BLOCK_SIZE != 0) return(EINVAL);	/* POSITION has to be a multiply of BLOCK_SIZE */
  cur_sector = m_ptr->POSITION / SECTOR_SIZE;	/* calculate cur_sector Bug22 */
  if (cur_sector >= HC_SIZE) return(EOF);	/* sector is beyond end of 1.2M disk */
  d = fp->fl_density;		/* diskette/drive combination. For now, d == 0 */
  fp->fl_cylinder = (int) (cur_sector / (NR_HEADS * nr_sectors_track[d]));
  fp->fl_sector = (int) interleave[cur_sector % nr_sectors_track[d]];	/* which sector in current track */
  fp->fl_head = (int) (cur_sector % (NR_HEADS*nr_sectors_track[d]) )/nr_sectors_track[d];
  fp->fl_count = m_ptr->COUNT;	/* how many bytes to transfer  */
  fp->fl_address = (vir_bytes) m_ptr->ADDRESS;	/* memory address to go for floppy controller's DMA internal buffer */
  fp->fl_procnr = m_ptr->PROC_NR;
  if (fp->fl_count != BLOCK_SIZE) return(EINVAL);	/* one block at a time to read/write */
  ...
  while (errors <= MAX_ERRORS) {
	...
	errors++;		/* increment count once per loop cycle */
 	if (errors % (MAX_ERRORS/NT) == 0) {
  	...
  	if (cur_sector >= total_sectors[d]) continue;	/* it can happen if we are at a wrong density */
 ...
 */
  ...
  ct = (vir_bytes) fp->fl_count;	/* Note that DMA is 1K buffer */
  user_phys = umap(proc_addr(fp->fl_procnr), D, vir, ct);
  low_addr  = (int) (user_phys >>  0) & BYTE_MASK;
  high_addr = (int) (user_phys >>  8) & BYTE_MASK;
  top_addr  = (int) (user_phys >> 16) & BYTE_MASK;
  low_ct  = (int) ( (ct - 1) >> 0) & BYTE_MASK;
  high_ct = (int) ( (ct - 1) >> 8) & BYTE_MASK;
  ...
  top_end = (int) (((user_phys + ct - 1) >> 16) & BYTE_MASK);
  if (top_end != top_addr) panic("Trying to DMA across 64K boundary", top_addr);	/* note that more than 16bit is more than 64K
											 * refer to 'cmpb    ah, dh' in build/bootblok.s  
											 */
 ...
 */
  ...
  lock();			/* no interrupts while checking out motor. you can think of lock() as mutex lock 
				 * and motor_status is shared resource 
				 */
  motor_bit = 1 << (fp->fl_drive + 4);	/* bit mask for this drive */
  motor_goal = motor_bit | ENABLE_INT | fp->fl_drive;
  if (motor_status & prev_motor) motor_goal |= prev_motor;	/* Bug24 */
  ...
  clock_mess(mtr_setup[d], send_mess);	/* motor was not running. We need time to wait for motor being started */
  receive(CLOCK, &mess);		/* wait for clock interrupt, alarm */
}
...
/* The drive is now on the proper cylinder.  Read or write 1 block. */
  ...
  /* The PC-AT requires the data rate to be set to 250 or 500 kbps */
  ...
  fdc_out(nr_sectors_track[d]);	/* tell controller how big a track is */
  ...
  s =  (fp->fl_results[ST_CYL] - fp->fl_cylinder) * NR_HEADS * nr_sectors_track[d];
  s += (fp->fl_results[ST_HEAD] - fp->fl_head) * nr_sectors_track[d];
...
/* Extract results from the controller after an operation. */
  ...
  for (i = 0; i < MAX_RESULTS; i++) {
	...
	fp->fl_results[i] = status & BYTE_MASK;
  }
 ...
 */
  ...
  while (retries-- > 0) {
	...
	r &= (MASTER | DIRECTION);	/* just look at bits 2 and 3 Bug23 */
 ...
 */
  ...
  motor_status = 0;
  motor_goal = 0;
  /* lock() is needed due to this port_out() 
   * Imagine if interrupt occurs between two port_out. 
   * Command doesn't execute correctly.
   */
  port_out(DOR, 0);		/* strobe reset bit low */
  port_out(DOR, ENABLE_INT);	/* strobe it high again
				 * These two commands like turning off switch and turn it on again
				 */

  unlock();			/* interrupts allowed again */
  receive(HARDWARE, &mess);	/* collect the RESET interrupt. TODO: need to know if it's interrupt from floppy?  */

  /* Interrupt from the reset has been received.  Continue resetting. Bug23 */
  fp = &floppy[0];		/* use floppy[0] for resetting */
  fp->fl_results[0] = 0;	/* this byte will be checked shortly */
  fdc_out(FDC_SENSE);		/* did it work? */
  r = fdc_results(fp);		/* get results */
  status = fp->fl_results[0] & BYTE_MASK;
