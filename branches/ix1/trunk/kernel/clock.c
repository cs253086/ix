...
#define IBM_FREQ    1193182L    /* IBM default clock frequency for setting timer */
#define SQUARE_WAVE     0x36    /* mode for generating square wave */
#define MILLISEC         100	/* how often to call the scheduler (msec) */
#define SCHED_RATE (MILLISEC * CLOCK_HZ / 1000)	/* number of ticks per schedule */
 ...
 */
  ...
  proc_nr = m_ptr->CLOCK_PROC_NR;       /* process that wants alarm. process to interrupt later */
  delta_ticks = m_ptr->DELTA_TICKS;     /* how many ticks to wait */
  function = m_ptr->FUNC_TO_CALL;       /* function to call (tasks only) */
  rp = proc_addr(proc_nr);

  mc.SECONDS_LEFT = (rp->p_alarm == 0L ? 0 : (rp->p_alarm - realtime) / CLOCK_HZ ); /* It is used in /mm/signal.c */
  rp->p_alarm = (delta_ticks == 0L ? 0L : realtime + delta_ticks); /* set alarm time */
  if (proc_nr < 0) watch_dog[-proc_nr] = function;	/* set a function to watch dog for task */
...
/* Get and return the current clock time in ticks. */
  ...
  mc.m_type = REAL_TIME;        /* set message type for reply. Bug20 */
  mc.NEW_TIME = boot_time + realtime / CLOCK_HZ;        /* current real time */
}
 ...
 *                              do_set_time                                  *
 *===========================================================================*/
static do_set_time(m_ptr)
message *m_ptr;                 /* pointer to request message */
{
/* Set the real time clock.  Only the superuser can use this call. */

  boot_time = m_ptr->NEW_TIME - realtime / CLOCK_HZ;
}
...
/* This routine called on every clock tick. */
  ...
  lost_ticks -= t;              /* these interrupts are no longer missed 
				 * Shouldn't it lost_ticks == 0? 
				 * No, lost_ticks can be increased between the statements
				 */

  /* Alarm system call handling */
  if (next_alarm <= realtime) {	/* if realtime reaches next_alarm. 
				 * Presicsely, next_alarm == realtime is correct 
				 * However, for robust system, next_alarm <= realtime is better 
				 */
  	/* It reaches next_alarm, but the process may have already exited/be killed, so check. */
  	next_alarm = MAX_LONG;       
	/* Search for the process having the 'next_alarm' value */ 
	for (rp = &proc[0]; rp < &proc[NR_TASKS+NR_PROCS]; rp++) {
		if (rp->p_alarm != (real_time) 0) {
			/* See if this alarm time has been reached. */
			if (rp->p_alarm <= realtime) {
				/* Found it! 
				 * If it is a user proc,
			...
			/* Work on determining which alarm is next. Pick the nearest alarm */
  ...
  if (--sched_ticks == 0) {
        ...
        sched_ticks = SCHED_RATE;               /* reset quantum. SCHED_RATE can be set to anything */
        prev_ptr = bill_ptr;                    /* new previous process 
						 * Note that bill_ptr refers to user process
						 * as well as prev_ptr 
						 */
 ...
 * kept pointing to a user process to charge for CPU usage.  If the CPU was in
 * user code prior to this clock tick, charge the tick as user time, otherwise
 * charge it as system time for FS/MM.
 * TODO: this only accounts one clock tick, but what about lost ticks. So
 * it's not that accurate. May need improvement
 */
...
static init_clock()
{
/* Initialize channel 0 of the 8253A timer to e.g. 60 Hz. http://en.wikibooks.org/wiki/X86_Assembly/Programmable_Interval_Timer */

  unsigned int count, low_byte, high_byte;

  count = (unsigned) (IBM_FREQ / CLOCK_HZ);     /* custom value to load into the timer: custom clock frequency is 60HZ
					 * It means every 1/60 second, clock interrupt occurs 
					 */
  low_byte = count & BYTE_MASK;              /* compute low-order byte */
  high_byte = (count >> 8) & BYTE_MASK;      /* compute high-order byte */
